...
import { Prisma } from "@integrador/db";
import { ApChatRealGateway, BlingOAuthHttpGateway, BlingRealGateway, BlingTokenRefreshCoordinator, } from "@integrador/integrations";
export class ProductionIntegrationProcessor {
        ...
        `);
          ...
          `);
                ...
                if (invoice.contact?.name) {
                    ...
                    const people = await transaction.$queryRaw(Prisma.sql `
          ...
          `);
                    ...
                    if (personId === null) {
                        const inserted = await transaction.$queryRaw(Prisma.sql `
              INSERT INTO pessoa (
                id_bling,
                nome,
                numero_documento,
            ...
            `);
                    ...
                    if (invoice.contact.address) {
                        const addresses = await transaction.$queryRaw(Prisma.sql `
    ...
    async syncProducts(context) {
        ...
        for (let page = 1; page <= 9; page += 1) {
            ...
            factoryGroups.push(...groups.filter((group) => normalizedLabel(group.name) === "fabricacao propria" ||
        ...
        const windows = await this.database.$queryRaw(Prisma.sql `
      SELECT
        ...
        await this.database.$transaction(async (transaction) => {
            for (const product of products) {
                if (!product.summary.cost) {
                    missingCost.push(product.summary.code ?? product.summary.name ?? product.summary.id);
                }
                const ownProduction = product.detail.productGroupId
                    ? factoryGroups.some((group) => group.id === product.detail.productGroupId)
                    : false;
                const rows = await transaction.$queryRaw(Prisma.sql `
    ...
    async syncSalesOrders(context, input) {
        const unitId = await this.#productionUnit(context.tenantId);
        const orders = [];
        let pages = 0;
        ...
        await this.#auditSynchronization(context, "bling.sales_orders.synchronized", {
            from: input.from,
            to: input.to,
    ...
    async syncNfeDetails(context, nfeId) {
        const tenant = await this.database.tenant.findUnique({
            where: { id: context.tenantId },
            select: { legacyUnitId: true, demo: true, active: true },
        });
        if (!tenant?.active || tenant.demo || tenant.legacyUnitId === null)
            throw new BadRequestError("Tenant sem unidade produtiva ativa");
        const invoices = await this.database.$queryRaw(Prisma.sql `
      ...
      `);
        ...
        const contact = detail.contactId
            ...
            : null;
        const boletoPayment = detail.paymentMethodIds.length === 0
            ? false
        ...
        for (const account of boletoBatch.accounts) {
            receivables.set(account.id, await this.#bling.getReceivableDetail(context, account.id));
        }
        const trackingCodes = [];
        if (isCorreios(detail.carrierName)) {
            for (const objectId of detail.logisticObjectIds) {
                ...
                if (logistic.trackingCode)
                    trackingCodes.push(logistic.trackingCode);
            }
        }
    ...
    async deliverApChat(context, message) {
        const tenant = await this.database.tenant.findUnique({
            where: { id: context.tenantId },
            select: { legacyUnitId: true, demo: true, active: true },
        });
    ...
    async #auditSynchronization(context, action, metadata) {
        await this.database.auditLog.create({
            data: {
                tenantId: context.tenantId,
                actorUserId: null,
...
class LegacyApChatCredentialProvider {
    database;
    constructor(database) {
        this.database = database;
    }
...
class LegacyBlingTokenRepository {
    database;
    constructor(database) {
        this.database = database;
    }
...
class LegacyBlingCredentialProvider {
    database;
    constructor(database) {
        this.database = database;
    }
...
class RefreshingBlingTokenProvider {
    tokens;
    refresh;
    constructor(tokens, refresh) {
        this.tokens = tokens;
        this.refresh = refresh;
    }
    async getAccessToken(tenantId, correlationId) {
        let token = await this.tokens.findByTenant(tenantId);
        if (!token)
            throw new BadRequestError("Integração Bling sem token ativo");
        if (token.status !== "S" ||
            token.expiresAtEpochSeconds <= Math.floor(Date.now() / 1_000) + 120) {
            await this.refresh.refreshIfNeeded(token.status === "R" ? 401 : "expires", {
                tenantId,
                correlationId,
                demo: false,
            });
    ...
    async handleUnauthorized(tenantId, correlationId) {
...
class PrismaAdvisoryLock {
    database;
    constructor(database) {
        this.database = database;
    }
    runExclusive(key, ttlMs, operation) {
