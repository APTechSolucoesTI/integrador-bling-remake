      ...
      this.database.$queryRaw<ExecutiveMetricRow[]>(Prisma.sql`
        SELECT ROUND(COALESCE(SUM(n.valor),0),2)::text revenue,
          ...
          ROUND(
            ...
            - ROUND(COALESCE(SUM(n.impostos),0),2)
            - ROUND(COALESCE(SUM(n.taxa),0),2)
            - ROUND(COALESCE(SUM(n.frete),0),2)
            - ROUND(COALESCE(SUM(n.custo_liquido),0),2)
            - ROUND(COALESCE(SUM(n.lucro),0),2),
            2
          )::text AS "otherExpenses",
    ...
    const [items, counts] = await Promise.all([
      this.database.$queryRaw<ExecutiveInvoiceRow[]>(Prisma.sql`
        SELECT n.id, n.numero::text number,
          ...
          ROUND(COALESCE(n.outras_despesas,0),2)::text AS "otherExpenses",
