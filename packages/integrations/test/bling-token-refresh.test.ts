...
class MemoryTokens implements BlingTokenRepository {
  constructor(public value: BlingTokenRecord | null) {}
  findByTenant(): Promise<BlingTokenRecord | null> {
...
class SerialLock implements DistributedLock {
...
const initial: BlingTokenRecord = {
  tenantId: "tenant-a",
...
function coordinator(tokens: MemoryTokens, oauth: BlingOAuthGateway) {
  const auditRecord = vi.fn(() => Promise.resolve());
  const audit: BlingRefreshAudit = { record: auditRecord };
  return {
    auditRecord,
    service: new BlingTokenRefreshCoordinator({
      ...
      credentials: {
        getCredentials: () =>
          Promise.resolve({ clientId: "client", clientSecret: "secret" }),
      },
      now: () => 1_000,
    }),
  };
...
describe("refresh distribuído do token Bling", () => {
  ...
  it("invalid_grant marca a integração como revogada", async () => {
    ...
    await expect(service.refreshIfNeeded("expires", context)).resolves.toBe(
      false,
    );
    expect(tokens.value?.status).toBe("N");
    expect(auditRecord).toHaveBeenCalledWith(
      expect.objectContaining({ outcome: "revoked", code: "invalid_grant" }),
    );
  });
  ...
  it("falha transitória restaura o estado disponível sem trocar tokens", async () => {
    ...
    const { service } = coordinator(tokens, {
      refresh: () =>
        Promise.resolve({ kind: "transient_failure", code: "http_503" }),
    });
